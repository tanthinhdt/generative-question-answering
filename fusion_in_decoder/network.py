...
class EncoderLayer(nn.Module):
    ...
    def __call__(self, inputs, encoder_mask=None, deterministic=False):
        ...
        encoder_bias = self.relative_embedding(inputs.shape[-2],
                                               inputs.shape[-2],
                                               True)
...
class DecoderLayer(nn.Module):
    ...
    def __call__(self,
        ...
        # inputs: embedded inputs to the decoder
        #         with shape [batch, length, emb_dim]
        x = layers.LayerNorm(
            dtype=cfg.dtype, name='pre_self_attention_layer_norm')(inputs)
...
class Encoder(nn.Module):
    ...
    def __call__(self,
        ...
        return nn.Dropout(rate=cfg.dropout_rate)(x,
                                                 deterministic=deterministic)
...
class Transformer(nn.Module):
    ...
    def encode(self,
        ...
        encoder_mask = layers.make_attention_mask(
            encoder_input_tokens > 0, encoder_input_tokens > 0,
            dtype=cfg.dtype
        )
        # Add segmentation block-diagonal attention mask
        # if using segmented data.
        ...
        return self.encoder(
            encoder_input_tokens, encoder_mask,
    ...
    def decode(
        ...
        if decode:
            ...
            encoder_decoder_mask = layers.make_attention_mask(
                decoder_target_tokens > 0, encoder_input_tokens > 0,
                dtype=cfg.dtype
            )

        # Add segmentation block-diagonal attention masks
        # if using segmented data.
        if encoder_segment_ids and decode:
            raise ValueError(
